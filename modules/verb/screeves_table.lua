...
export.names = {
    ...
    ["imperative__brZandebiTi"] = "ბრძანებითი",
}
            ...
            for _, person in pairs({"I", "II", "III"}) do
                if self[screeve][person] == nil or type(self[screeve][person]) ~= "table" then
                    ...
                    for _, number in pairs({"singular", "plural"}) do
                        if self[screeve][person][number] == nil or type(self[screeve][person][number]) ~= "string" then
                            error("ERROR 3: in Screeeve '" .. screeve .. "', person '" .. person .. "', number '" .. number .. "': must be a string")
                        end
                    end
                end
            end
